...
% QR links to the full report text (not the source repository).
\newcommand{\reporturl}{https://pakhomovee.github.io/project.html?id=repa-diffusion}
...
\institute[HSE]{HSE University \quad---\quad Methods of Optimization in Machine Learning}

\footercontent{\href{mailto:evgenii.research@gmail.com}{evgenii.research@gmail.com} \hfill Course Project \hfill \url{\reporturl}}
\logoright{\includegraphics[height=4.8cm]{hse_logo.pdf}}
...
frozen encoder (DINOv2). We ask \emph{when} this helps --- and show a constant alignment weight eventually hurts.
\end{block}
...
$\lambda\ge0$ --- the adapted gradient never works against the diffusion descent direction. Parameter-free.
\end{block}

\begin{block}{Toy Model: Whitened Linear Setting}
Clean sample $x_0\in\mathbb R^d$; whitened features $u$ with $\mathbb E[uu^\top]=I$; linear denoiser $\hat x_0=Bu$.
The diffusion loss $L_{\mathrm{diff}}(B)=\tfrac12\mathbb E\|Bu-x_0\|^2$ has the unique optimum
$A=\mathbb E[x_0u^\top]$. A teacher projects onto an $r$-dim semantic subspace ($P^2=P$, $Q=I-P$), giving
$L_{\mathrm{repa}}(B)=\tfrac12\mathbb E\|Bu-Px_0\|^2$ --- it keeps the semantic part $Px_0$ and \emph{drops} the
detail $Qx_0$.
...
denoiser $A$ while still using the alignment signal.
\end{block}
...
Baseline / REPA / REPA-$\Sigma$ ($\pm\,\lambda$-anneal) on three single-domain datasets spanning a
spatial-difficulty axis. On \textbf{CelebA}, surgery helps mid-training and $\lambda$-annealing late; combining
...
PCA feature maps show REPA preserves semantic, face-centred structure through the mid-network and under heavy
noise, where the baseline collapses. We extend the comparison to all three domains.
\begin{figure}
  \centering
...
\begin{center}
  ...
  {\small Scan for the full report}
\end{center}
\end{alertblock}
