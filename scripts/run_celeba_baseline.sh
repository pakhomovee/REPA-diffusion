#!/usr/bin/env bash
# Run CelebA baseline (no REPA encoder alignment) training.
# Prerequisites:
#   1. Export CelebA images:  python scripts/export_celeba_for_repa.py
...
# Default export uses 4 attributes (Male, Smiling, Young, Attractive) => 16 classes.
...
${ACCELERATE:-accelerate} launch \
  ...
  --exp-name="${EXP_NAME:-celeba_sit_b2_baseline}" \
