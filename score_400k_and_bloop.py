#!/usr/bin/env python3
"""
score_400k_and_bloop.py — FID/KID for 30 checkpoints:
  - baseline 200k→400k extension @ every 20k  (10 ckpts)
  - REPA-const 200k→400k extension @ every 20k (10 ckpts)
  - REPA-Σ bloop (EMA-stabilized) 0→200k @ every 20k (10 ckpts)

Mirrors the robust pattern of score_anneal_robust.py:
  - PRIORITY-ORDERED npz acquisition (cache → PNG recovery → DDP → DDP+recovery)
  - CSV-based resume
  - InceptionV3 built once
  - Inter-run REPA/samples/ wipe

DIFFERENCE: after each successful (acquire + score), we DELETE the npz to keep
disk usage bounded to ~9 GB peak per ckpt (per user request "не занимать много
памяти"). The CSV row is the durable artifact.

Launch:
  ssh ya100
  cd <repo-root>
  tmux new -d -s fid_400k_bloop '
    cd <repo-root>
    export CUDA_VISIBLE_DEVICES=1,2,3,4,5,6
    export NUM_GPUS=6
    python score_400k_and_bloop.py 2>&1 \
      | tee -a logs/fid_400k_and_bloop.log
  '
"""
...
# ── Configuration ─────────────────────────────────────────────────────────────
# Per-run STEPS: extensions cover 220k-400k @ 20k; bloop covers 20k-200k @ 20k.
RUNS = [
    (
        "Baseline",
        "celeba_sit_b2_baseline_gpus2-7_40k",
        list(range(220_000, 401_000, 20_000)),  # 220k..400k
    ),
    (
        "REPA (DINOv2-B)",
    ...
    (
        ...
        list(range(20_000, 201_000, 20_000)),   # 20k..200k @ 20k granularity
    ),
]
